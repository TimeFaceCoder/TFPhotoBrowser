//
//  TFCollectionPickerController.m
//  TFPhotoBrowser
//
...
#import "TFCollectionCell.h"
#import "PHCollection+TFThumbnail.h"


@interface TFCollectionPickerController () <PHPhotoLibraryChangeObserver, UIViewControllerRestoration>
{
...
@end
@implementation TFCollectionPickerController
...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[TFCollectionCell class] forCellReuseIdentifier:@"CollectionCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < [self _momentsSections]) {
        ...
        if ([collection isKindOfClass:[PHAssetCollection class]]) {
            ...
            cell.thumbnailView.image = TFPhotoBrowserImageNamed(@"TFLibraryCollectionList");
            [collection tf_requestThumbnailWithAssetsFetchOptions:self.assetFetchOptions
                                                       completion:^(UIImage *result)
             {
                 if (result != nil) {
                     cell.thumbnailView.image = result;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < [self _momentsSections]) {
        ...
        if ([collection isKindOfClass:[PHAssetCollection class]]) {
            ...
            TFCollectionPickerController *picker = [[TFCollectionPickerController alloc] init];
...
- (void)decodeRestorableStateWithCoder:(NSCoder *)coder
{
    ...
    id<TFCollectionPickerControllerDelegate> delegate = [coder decodeObjectForKey:@"delegate"];
    if (delegate != nil) {
