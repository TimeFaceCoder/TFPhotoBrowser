...
- (void)viewDidLoad {
    ...
    if(!_doneButtonImage) {
        ...
        [_doneButton setTitle:TFPhotoBrowserLocalizedStrings(@"Done") forState:UIControlStateNormal];
    ...
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                ...
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
                    TFPLog(@"Pre-loading image at index %i", pageIndex-1);
                }
            }
...
- (void)tilePages {
    ...
    for (TFZoomingScrollView *page in _visiblePages) {
        ...
        if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
            ...
            TFPLog(@"Removed page at index %i", PAGE_INDEX(page));
        }
    }
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            TFPLog(@"Added page at index %i", index);
...
- (void)updateToolbar {
    ...
    if ([self numberOfPhotos] > 1) {
        _counterLabel.text = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), TFPhotoBrowserLocalizedStrings(@"of"), (unsigned long)[self numberOfPhotos]];
    } else {
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        else
        {
            ...
            self.actionsSheet.cancelButtonIndex = [self.actionsSheet addButtonWithTitle:TFPhotoBrowserLocalizedStrings(@"Cancel")];
